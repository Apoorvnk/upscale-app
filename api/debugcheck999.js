...
export default async function handler(req, res) {
  ...
  try {
    ...
    const t4 = await client.from("marketing_pages").select("id, phone, angle, pitch, strategy, tactics").limit(1);
    results.marketing_pages = { ok: !t4.error, error: t4.error?.message };
  } catch (err) {
